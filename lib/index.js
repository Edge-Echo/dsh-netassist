...
export default Object.assign(function netassist(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Reading system proxy settings', kind: 'read' }),
        presentResult: (_a, result) => {
            const m = meta(result);
            return {
                card: 'generic',
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Probing local proxy ports', kind: 'search' }),
        presentResult: (_a, result) => {
            const m = meta(result);
        ...
        async execute(args) {
            const ports = args.ports ?? [10808, 10809, 7890, 7897, 8888, 1080];
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: (args) => ({
            card: 'generic',
            title: `Diagnosing ${args.host ?? 'host'}`,
            kind: 'fetch',
        }),
        presentResult: (_a, result) => {
            const m = meta(result);
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Scanning hosts file for GitHub entries', kind: 'search' }),
        presentResult: (_a, result) => {
            const m = meta(result);
            return {
                card: 'generic',
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Running network doctor', kind: 'search' }),
        presentResult: (_a, result) => {
            const m = meta(result);
        ...
        async execute() {
            const report = await runDoctor({ timeoutMs });
            return {
                ok: report.ok,
