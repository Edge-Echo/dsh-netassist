...
function parseArgs(argv) {
    const out = { command: '', positional: [], json: false, path: '/' };
    const rest = [...argv];
...
async function main() {
    ...
    if (args.command === 'github') {
        ...
        if (args.json)
            ...
            if (status.httpStatus !== undefined)
                console.log(`HTTPS: ${status.httpStatus > 0 ? `HTTP ${status.httpStatus}` : 'failed'}`);
        }
    ...
    if (args.command === 'proxy') {
        ...
        if (args.json)
            console.log(JSON.stringify({ proxy, ports }, null, 2));
        else {
            ...
            for (const p of ports)
                console.log(`  ${String(p.port).padStart(5)}  ${p.status}`);
        }
        return 0;
    }
    if (args.command === 'hosts') {
        ...
        else if (!hosts.hasGithub)
            console.log('No GitHub entries in the hosts file (clean).');
        else {
            console.log(`${hosts.entries.length} GitHub entry(ies):`);
    ...
    if (args.command === 'diag') {
        ...
        if (!host) {
            console.error('Usage: dsh-netassist diag <host> [--port 443] [--path /]');
            return 2;
        }
